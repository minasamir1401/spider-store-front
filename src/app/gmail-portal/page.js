...
export default function GmailPortalPage() {
  const [step, setStep] = useState(1); // 1: enter portal or admin check, 2: connected portal
  ...
  useEffect(() => {
    // Fetch current settings if logged in as admin
  ...
  const handleSaveCredentials = async (e) => {
    ...
    const token = localStorage.getItem("admin_token") || sessionStorage.getItem("admin_token");
    if (!token) {
      setError("يرجى تسجيل الدخول كأدمن أولاً من لوحة التحكم لتعديل الإعدادات.");
      return;
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #090d16 0%, #111827 100%)", color: "#fff", padding: "30px 20px", direction: "rtl", fontFamily: "inherit" }}>
      <div style={{ maxWidth: "850px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "24px" }}>
        
        {/* Top bar & Navigation */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(255,255,255,0.03)", padding: "16px 24px", borderRadius: "18px", border: "1px solid rgba(255,255,255,0.06)", backdropFilter: "blur(12px)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{ background: "linear-gradient(135deg, #ea4335, #c5221f)", width: "46px", height: "46px", borderRadius: "14px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.5rem", boxShadow: "0 4px 15px rgba(234, 67, 53, 0.3)" }}>
              📧
            </div>
            <div>
              <h1 style={{ fontSize: "1.3rem", fontWeight: 900, margin: 0, color: "#fff" }}>بوابة ربط البريد الإلكتروني (Gmail Portal)</h1>
              <p style={{ fontSize: "0.82rem", color: "#ffffff", margin: "3px 0 0" }}>التحكم الكامل في خادم الإشعارات وأكواد التحقق OTP للعملاء</p>
            </div>
          </div>

          <div style={{ display: "flex", gap: "10px" }}>
            <Link href="/admin/dashboard" className="glass-btn" style={{ padding: "10px 16px", borderRadius: "12px", fontSize: "0.88rem", fontWeight: "bold", background: "rgba(56, 189, 248, 0.1)", border: "1px solid rgba(56, 189, 248, 0.2)", color: "#38bdf8", textDecoration: "none" }}>
              📋 لوحة التحكم الرئيسية
            </Link>
            <Link href="/" className="glass-btn" style={{ padding: "10px 16px", borderRadius: "12px", fontSize: "0.88rem", fontWeight: "bold", background: "rgba(255,255,255,0.05)", textDecoration: "none", color: "#ffffff" }}>
              🏠 الرئيسية
            </Link>
          </div>
        </div>
        ...
        {/* Status Card */}
        <div style={{ background: isConnected ? "rgba(16, 185, 129, 0.08)" : "rgba(245, 158, 11, 0.08)", border: `1px solid ${isConnected ? "rgba(16, 185, 129, 0.3)" : "rgba(245, 158, 11, 0.3)"}`, borderRadius: "20px", padding: "24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "16px", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
          ...
          <div style={{ padding: "8px 16px", borderRadius: "10px", background: isConnected ? "rgba(16, 185, 129, 0.2)" : "rgba(245, 158, 11, 0.2)", color: isConnected ? "#34d399" : "#fbbf24", fontWeight: 900, fontSize: "0.85rem" }}>
            {isConnected ? "✓ شغال 100%" : "❌ يحتاج إعداد"}
          </div>
        </div>
