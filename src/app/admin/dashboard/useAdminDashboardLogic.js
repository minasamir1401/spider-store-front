...
export function useAdminDashboardLogic(API_BASE_URL) {
  ...
  const fetchData = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      if (settingsRes.ok) {
        ...
        if (settingsData.home_stats) {
          try {
            setHomeStats(typeof settingsData.home_stats === 'string' ? JSON.parse(settingsData.home_stats) : settingsData.home_stats);
          } catch(e) {
            console.error(e);
          }
        }
...
const handleLogout = () => {
  ...
  return {
    hydrated, activeTab, setActiveTab, adminDrawerOpen, setAdminDrawerOpen,
    orders, categories, services, walletRequests, walletTransactions, customers,
    banners, homeStats, paymentMethodsList, globalCurrencies, exchangeRates,
    baseCurrency, unlockerServices, unlockerCategories,
    dashboardContextValue,
    orderSearch, setOrderSearch, orderFilter, setOrderFilter,
    catSearch, setCatSearch, serviceSearch, setServiceSearch,
    walletSearch, setWalletSearch, walletFilter, setWalletFilter,
    customerSearch, setCustomerSearch, bannerSearch, setBannerSearch,
    filteredOrders, filteredCategories, filteredServices, filteredWalletRequests,
    filteredWalletTransactions, filteredCustomers,
    handleLogout: () => {
      localStorage.removeItem("admin_token");
      localStorage.removeItem("admin_user");
